...
public class BasesDestroyedAlert extends GameAlert {
    ...
    public String description() {
        if (spy == null)
            return text("MAIN_ALERT_BASES_DESTROYED2", str(count), player().sv.name(system.id));
        else
            return text("MAIN_ALERT_BASES_DESTROYED", spy.name(),str(count), player().sv.name(system.id));
    }
