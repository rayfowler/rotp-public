...
public class DesignEcmSelectionUI extends DesignSelectionUI {
    private static final long serialVersionUID = 1L;
    @Override
    String title()                    { return text("SHIP_DESIGN_ECM_TITLE"); }
    @Override
    ...
    @Override
    List<? extends ShipComponent> baseComponents() { return player().shipLab().ecms(); }
    @Override
    ShipComponent selectedComponent()              { return selectedDesign.ecm(); }
    @Override
    void select(int compNum)   { selectedDesign.ecm((ShipECM)components().get(compNum)); }
}